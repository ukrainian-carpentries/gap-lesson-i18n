...
якщо ви повернетеся до нього в майбутньому. Коментар у GAP починається з символу `#` та
...
Тоді після натискання клавіші Return GAP відобразить нове запрошення, але коментар
...
якщо також хочемо їх записати. Замість того, щоб вводити їх повторно, ми будемо використовувати клавіші зі стрілками вгору та вниз
для перегляду _історії командного рядка_. Натискайте ці клавіші, щоб побачити попередні команди. Коли побачите потрібну
формулу, натисніть Return (розташування курсору в командному
...
Ви також можете редагувати команди, які були використані раніше. Натисніть клавішу зі стрілкою вгору ще раз, а потім використовуйте
клавіші зі стрілками вліво та вправо, Delete або Backspace, щоб відредагувати цю команду та замінити 32 на 64. Іншими корисними комбінаціями клавіш є Ctrl-A та Ctrl-E, які переміщують курсор на початок і кінець
...
Якщо історія командного рядка велика, можна
виконати частковий пошук, ввівши початкову частину команди, а потім використовуючи
клавіші зі стрілками вгору та вниз, переглянути лише ті рядки, які починаються
з введених символів.

Якщо ви бажаєте зберегти значення для подальшого використання, йому можна присвоїти ім'я
за допомогою `:=`
...
- GAP використовує `=`, щоб перевірити, чи два об'єкти дорівнюють один одному (інші мови можуть використовувати для цього `==`).

- Нарешті, GAP використовує `<>`, щоб перевірити, що два об'єкти не рівні (замість `!=`, що ви могли бачити в інших мовах програмування).

::::::::::::::::::::::::::::::::::::::::::::::::::

Пробільні символи (тобто пробіли, табуляції та символи переводу рядка) не мають значення в GAP,
за винятком випадків, коли вони знаходяться всередині рядка. Наприклад, попередню команду
...
для кращої читабельності. Наприклад, наступна команда створює матрицю 3×3:
...
Замість цього ми можемо записати нашу матрицю в 3 рядки. У цьому випадку замість повного запрошення
`gap>` відображатиметься часткове запрошення `>`, доки користувач не завершить
...
наприклад, введіть `LogTo;` та `Factorial;`
...
функція `Filtered` сприймає список і функцію, повертаючи
всі елементи списку, які задовольняють умову, яку перевіряє функція.
Зовсім не дивно, що функція `IsEvenInt` ("Is Even Integer" з англ. "чи є ціле число парним") перевіряє, чи є ціле число парним:
...
Корисною функцією інтерфейсу командного рядка GAP, яка економить час, є автодоповнення
ідентифікаторів під час натискання клавіші Tab. Наприклад, введіть `Fib`, а потім
натисніть клавішу Tab, щоб автоматично завершити введений текст до `Fibonacci`:
...
або `NaturalHomomorphismByNormalSubgroup` за допомогою автодоповнення.
...
задокументовані в документації GAP [тут](https://www.gap-system.org/Manuals/doc/ref/chap5.html#X81F732457F7BC851).
...
тому що назва цієї функції GAP – `Factorial`. Використання малих літер
замість великих або навпаки також впливає на автодоповнення назви.

Тепер розглянемо таку задачу: для скінченної групи _G_ обчислити
...
функцію `Group` та згадку про функцію `Order`.

Документація GAP доступна у кількох форматах: текстовий формат зручний для перегляду в терміналі,
PDF зручний для друку, а HTML (особливо з підтримкою MathJax)
дуже ефективний для перегляду за допомогою браузера. Якщо ви використовуєте GAP на власному комп’ютері, ви можете налаштувати GAP так, щоб він відкривав довідку за допомогою вашого браузера за замовчуванням. (це навряд чи спрацює, якщо ви використовуєте віддалене підключення). Дивіться `?WriteGapIniFile` про те, як зробити це налаштування постійним:
...
Давайте тепер скопіюємо наступні вхідні дані з першого прикладу з глави про групи у довідковому посібнику GAP. У ньому показано, як створювати перестановки та присвоювати значення
...
Ця документація показує, як перестановки в GAP записуються в циклічній нотації, а також
показує загальні функції, які використовуються з групами. Крім того, у деяких місцях використовуються дві крапки з комою
в кінці рядка. Це зупиняє зображення результатів обчислень.
...
Нашою наступною метою є з'ясувати, як отримати список елементів `G` та їх порядок.
Введіть `?elements` і перегляньте список тем довідки. Після перевірки, інформація з Tutorial не здається доречною, але вміст розділу у
довідковому посібнику (Reference) є корисним. Він також пояснює різницю між використанням `AsSSortedList` та `AsList`. Отже, так ми отримуємо список елементів `G`:
...
для дослідження та повторного використання. Ми забули це зробити, коли обчислювали його перший раз. Звичайно,
ми можемо використати історію командного рядка, щоб відновити останню команду, відредагувати
її та викликати знову. Але замість цього ми будемо використовувати `last`, яка є спеціальною змінною,
що містить останній результат, який повернув GAP:
...
- Можуть містити дірки або бути порожніми

- Можуть динамічно змінювати свою довжину (за допомогою `Add`, `Append` або прямого призначення)

- Не обов’язково містять об’єкти одного типу

- Дивіться більше в документації - [GAP Tutorial: Lists and Records](https://docs.gap-system.org/doc/tut/chap3.html)

::::::::::::::::::::::::::::::::::::::::::::::::::

Багато функцій у GAP посилаються на множини. Множина (`Set`) у GAP — це список,
який не має повторень та дірок, і у якому елементи розташовані у порядку зростання. Ось кілька прикладів:
...
Тепер давайте розглянемо цікаве обчислення - визначення середнього порядку елементів
групи `G`. Існує багато різних способів це зробити, і ми розглянемо тут деякі з них.

Цикл `for` у GAP дозволяє виконати якусь дію із кожним елементом колекції.
...
Наприклад, ми можемо знайти середній порядок елементів нашої групи `G` наступним чином:
...
Насправді, ми можемо просто перебирати елементи групи `G` (загалом GAP
дозволить вам перебирати елементи більшості типів об’єктів). Нам потрібно перейти на використання `Size`
...
Існують і інші способи організації циклів. Наприклад, натомість ми можемо перебирати цілі числа із відповідного діапазону
та розглядати `elts` як масив:
...
Однак часто існують і більш компактні способи виконання тієї ж самої операції. Ось дуже
короткий шлях отримати цей результат:
...
GAP має дуже корисний інструментарій для роботи зі списками. Зараз ми покажемо ще кілька прикладів.

Іноді GAP не має саме тієї функції, яка нам потрібна.
Наприклад, `NrMovedPoints` повертає кількість точок, які пересуває перестановка,
але що робити, якщо ми хочемо знайти всі перестановки, які пересувають 4 точки? Ось тут
і з’являється позначення "стрілка". Вираз `g -> e` створює нову функцію, яка отримує один аргумент `g`
і повертає значення виразу `e`. Ось деякі приклади:

- визначення всіх елементів `G` які не мають фіксованих точок:
...
- пошук перестановки в `G`, яка спрягає (1,2) та (2,3)
...
Давайте це перевіримо (пам’ятайте, що в GAP перестановки множаться зліва направо!):
...
- перевірка того, чи є елемент у `G`, який пересуває рівно дві точки:
