...
Тепер припустимо, що ми хочемо зберегти цей фрагмент коду GAP і пізніше
повторити ці обчислення для деяких інших груп. Ми навіть можемо переформатувати цей код,
...
- ще небезпечніше: можна забути повторно встановити `sum` на нуль перед новим
...
Тепер ми можемо створювати нові групи та повторно використовувати `AvgOrdOfGroup` для обчислення середнього
...
наступний код функції та коментарі (хороший шанс звернути увагу на коментування коду):
...
Очевидно, що функція `AvgOrdOfGroup` невизначена в цьому сеансі, тому спроба
викликати її призведе до помилки:
...
ми припускаємо, що файл `avgord.g` знаходиться в поточному каталозі, тому шлях до нього не потрібно вказувати.
...
У цьому прикладі ми навмисно розпочали новий сеанс GAP для того, щоб було зрозуміло,
...
файлу є складнішим). Повторний виклик `Read` виконує весь код у файлі, що зчитується. Якщо код функції було змінено і
...
перевизначена та залишається без змін:
...
Далі, давайте розглянемо приклад _попередження_. Оскільки це лише попередження, читання коду
...
Ці попередження означають, що `g` та `sum` не оголошено як `local` (локальні)
змінні. Тому GAP очікує, що вони будуть глобальними змінними
під час виклику функції. Оскільки вони не існували під час виклику `Read`, то
відображалося попередження. Однак, якби ці змінні вже існували
...
Перш ніж перейти до наступного епізоду, будь ласка,
